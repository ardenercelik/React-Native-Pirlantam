import Asscher from './asscher.svg';
import Cushion from './cushion.svg';
import Emerald from './emerald.svg';
import Heart from './heart.svg';
import Markiz from './markiz.svg';
import Oval from './oval.svg';
import Pear from './pear.svg';
import Princess from './princess.svg';
import Radiant from './radiant.svg';
import Round from './round.svg';
import SquareRadiant from './square-radiant.svg';
import NoData from './no-data.svg';
import NotFound from './not-found.svg';
import Search from './search.svg';
import TR from './TR.svg';
import Empty from './empty-street';
import UndrawLogin from './login';
export {Asscher, Cushion, Emerald, Heart, Markiz, Oval, Pear, Princess, Radiant, Round, SquareRadiant, NoData, NotFound, Search, TR, Empty, UndrawLogin};
